...
function move(amount, position, $, addAmount) {
    const idx = amount.indexOf("U") + amount.indexOf("D") === -1 ? 1 : 0;
    const dir = getDirection(amount);

    const len = +amount.substring(1, amount.length);

    for (let i = 0; i < len; ++i) {
        position[idx] += dir;
        position[2]++;

        const coords = position.slice(0, 2).join();
        if (!$[coords]) {
            $[coords] = 0;
        }

        $[coords] += addAmount * position[2];
    }
}
...
realA.split(',').forEach(x => move(x, p, $Money, addA));

p[0] = 0;
p[1] = 0;
p[2] = 0;
realD.split(',').forEach(x => move(x, p, $Money, addB));

console.log(Object.
    keys($Money).
    filter(x => $Money[x] >= addB && $Money[x] % addB > 0).
    /*
    reduce((reduce$, key) => {

        const dist = Math.floor($Money[key] / addB) + $Money[key] % addB;
        reduce$[key] = dist;
